...
class ScopeVar {
  constructor(runtime) {
    ...
    // this.runtime.on('COMPILE_ERROR', (target, error) => {
    //   console.error(`Could not compile script in ${target.getName()}: ${error}`);
    // });
    ...
    // this.runtime.on('PROJECT_LOADED', this.parseExtConfig.bind(this));
  }
  ...
  patchCompiler() {
    ...
    if (!dangerousExports) {
      console.log("Ext_ScopeVar compile patch failed, no exported compiler");
      return;
    }
  ...
  __listMenu() {
    const menus = [];
    // 放入全局列表
    let { variables } = this.runtime.getTargetForStage();
  ...
  _getExtConfig() {
    if (!this.runtime.extensionStorage) this.runtime.extensionStorage = {};
    const storage = this.runtime.extensionStorage;
    if (!storage[extensionId]) storage[extensionId] = {};
  ...
  _getVarObjByName(name, thread, isThreadVar = false) {
    ...
    for (let i = stackFrames.length - 2; i >= 0; i--) {
      ...
      if (
        ...
        name in executionContext.shikiVars
      ) {
    ...
    if (typeof thread.shikiVars === "object" && name in thread.shikiVars) {
      return thread.shikiVars;
    }
  ...
  /**
   * 遍历对象前初始化信息
   * @param {*} LIST  Scratch列表Id或名称 / 数组或对象 / 对象JSON
   * @param {*} target
   * @returns {{keys, n, list, type}}
   */
  _initForeachList(LIST, target) {
    ...
    if (typeof LIST === "object") {
      ...
      if (str === "empty") return null; // 未选择列表
      list = this.getListByIdOrName(str, target);
      if (list) {
        list = list.value;
        type = "list";
      } else {
    ...
    if (type === "obj") {
      frame.keys = Object.keys(list);
      frame.n = frame.keys.length;
    } else {
      frame.n = list.length;
    }
    frame.list = list;
  ...
  _getKVByIdx(idx, frame) {
    let k;
    let v;
    const { SafeObject } = this.runtime;
    const toSafeObject = (val) => (SafeObject ? SafeObject.toSafeObject(val) : val);
    if (frame.type === "obj") {
      const prop = frame.keys[idx];
      k = prop;
      v = toSafeObject(frame.list[prop]) ?? "";
    } else {
      k = idx + 1;
      v = toSafeObject(frame.list[idx]) ?? "";
    }
    return { k, v };
  }
  ...
  forEachWithList({ LIST, VAR, I }, util) {
    const { stackFrame } = util;
    if (typeof stackFrame.loopCounter === "undefined") {
      const info = this._initForeachList(LIST, util.target);
      if (!info) return;
      stackFrame.varName = Cast.toString(VAR);
      stackFrame.idxName = Cast.toString(I);
      stackFrame.loopCounter = 1;
      Object.assign(stackFrame, info);
    } else {
      stackFrame.loopCounter += 1;
    }
    if (stackFrame.loopCounter > stackFrame.n) {
    ...
    const vars = this._getOrInitScopeVars(util.thread, 0);
    const { k, v } = this._getKVByIdx(idx, stackFrame);
